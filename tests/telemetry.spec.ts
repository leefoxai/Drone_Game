...
test('쉬운 조종은 사람 입력과 물리 적용 입력을 함께 기록하고 assist version을 남긴다',()=>{
  const context:ControlContext={trackId:'training-five-v1',controlMode:'assisted',aircraftProfileVersion:profile.version,assistVersion:ASSIST_VERSION};
  ...
  recorder.record(0,pilot,applied,cameraSnapshot(75,1920,1080,1));
...
test('Acro와 쉬운 조종의 순위/학습 파티션 키가 분리된다',()=>{
  const assisted:ControlContext={trackId:'race-five-v1',controlMode:'assisted',aircraftProfileVersion:2,assistVersion:1};
  const acro:ControlContext={trackId:'race-five-v1',controlMode:'acro',aircraftProfileVersion:2,assistVersion:null};
  expect(dataPartitionKey(assisted)).not.toBe(dataPartitionKey(acro));
  const archive=new SessionLapArchive();
  const a=new LapTelemetryBuffer(assisted);a.record(0,{throttle:.5,roll:0,pitch:0,yaw:0},{throttle:.3,roll:0,pitch:0,yaw:0},cameraSnapshot(75,1280,720));archive.add(a.finalize(40,true));
  const b=new LapTelemetryBuffer(acro);b.record(0,{throttle:.3,roll:0,pitch:0,yaw:0},{throttle:.3,roll:0,pitch:0,yaw:0},cameraSnapshot(75,1280,720));archive.add(b.finalize(42,true));
  expect(archive.count(assisted)).toBe(1);expect(archive.count(acro)).toBe(1);expect(archive.partitionCount()).toBe(2);
});

test('카메라 기록은 세로 FOV와 화면 비율/크기를 함께 보존한다',()=>{
  const camera=cameraSnapshot(82,1920,1080,1.5);
