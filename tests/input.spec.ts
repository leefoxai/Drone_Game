...
test('조종기 기본 데드존 1%, 표준 게임패드 5%',()=>{
  expect(DEFAULT_DEADZONE).toEqual({controller:.01,gamepad:.05});
  expect(deviceKind({mapping:''} as Gamepad)).toBe('controller');
  expect(deviceKind({mapping:'standard'} as Gamepad)).toBe('gamepad');
  for(const channel of ['throttle','roll','pitch','yaw'] as const){
    expect(defaultMapping('controller')[channel].deadzone).toBe(.01);
