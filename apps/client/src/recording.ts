...
export interface RecordingMetadata extends ControlContext {
  ...
  camera: CameraSnapshot & { nearM: number; farM: number; relativePositionM: [number, number, number] | null; relativeOrientation: [number, number, number, number] | null };
...
export class M2LapRecorder {
  ...
  frame(record:Omit<FrameInputRecord,'frameSequence'>){if(this.active)this.frames.push({frameSequence:this.frameSequence++,...structuredClone(record)});}
