...
import type { ControlContext, ControlMode } from '../../../packages/physics/src/telemetry';
...
const context=():ControlContext=>({trackId:track.id,controlMode:flightMode,aircraftProfileVersion:profile.version,assistVersion:flightMode==='assisted'?ASSIST_VERSION:null});
...
let paused=false, source='keyboard', cameraMode='chase', readyFrames=0;
...
function applyContext(reason:string){
  race.setContext(track,dataPartitionKey(context()));recorder.setContext(context());syncCameraTilt();reset(reason);
}
...
el<HTMLSelectElement>('flight-mode').onchange=event=>{
  flightMode=(event.target as HTMLSelectElement).value as ControlMode;
...
el('panel-toggle').onclick=()=>{
  const panel=el('controls');panel.hidden=!panel.hidden;
  el('panel-toggle').textContent=panel.hidden?'설정 열기':'설정 접기';
  el('panel-toggle').setAttribute('aria-expanded',String(!panel.hidden));
};
...
const tune=(id:string,callback:(value:number)=>void,format:(value:number)=>string,physics=false)=>{
  ...
  field.oninput=()=>{
    ...
    callback(value);el(id+'-out').textContent=format(value);
    if(physics){race.invalidate();message('튜닝 즉시 적용 · 현재 랩 무효, R로 새 랩을 시작하세요.');}
  };
};
const fixed=(v:number)=>v.toFixed(2);
for(const [id,key] of [['rc-rate','rcRate'],['super-rate','superRate'],['expo','expo']] as const) {
  tune(id,value=>{profile.rates[key]=value;el('max-rate').textContent='최대 '+Math.round(rate(1,profile.rates)*180/Math.PI)+' °/s · 세 축 공통';},fixed,true);
}
...
function onKeyDown(event:KeyboardEvent) {
  ...
  if(!event.repeat){if(event.code==='KeyR')reset();if(event.code==='KeyP')togglePause();if(event.code==='KeyC')switchCamera();if(event.code==='KeyH')el('hover').click();}
  keys.add(event.code);
}
...
const time=(seconds:number)=>{
  const ms=Math.floor(seconds*1000);return String(Math.floor(ms/60000)).padStart(2,'0')+':'+String(Math.floor(ms/1000)%60).padStart(2,'0')+'.'+String(ms%1000).padStart(3,'0');
};
function updateUI(){
  el('altitude').innerHTML=state.position[1].toFixed(2)+' <em>m</em>';
  el('speed').innerHTML=Math.hypot(...state.velocity).toFixed(1)+' <em>m/s</em>';
  el('throttle-value').innerHTML=Math.round(input.throttle*100)+' <em>%</em>';
  el('battery').innerHTML=state.voltage.toFixed(1)+' <em>V</em>';
  el('lap-time').textContent=time(race.elapsed(state.tick));
  el('gate-progress').textContent='다음 게이트 '+(race.nextGate+1)+' / '+race.gateCount+(race.valid?'':' · 랩 무효');
  el('best-lap').textContent=race.bestSeconds===null?'이 모드 완료 랩 없음':'이 모드 최고 '+time(race.bestSeconds)+' · '+race.laps+'랩';
  const camera=recorder.latestCamera??cameraSnapshot(fov,Math.max(1,canvas.clientWidth),Math.max(1,canvas.clientHeight),devicePixelRatio);
  const telemetry=el('telemetry');
  ...
  const graphAxis=Number(el<HTMLSelectElement>('graph-axis').value);
  el('live-rate').textContent=(state.omega[graphAxis]!*180/Math.PI).toFixed(0)+' °/s';
  el('live-thrust').textContent=state.thrustN.toFixed(1)+' N';
  diagnostics.draw(graphAxis,state,race.nextGate,track);controller.updateDisplay();
}
function frame(now:number) {
  frameId=requestAnimationFrame(frame);
  const elapsed=lastTime?Math.max(0,(now-lastTime)/1000):0;lastTime=now;
  const padInput=controller.poll();
  if(readyFrames<3){readyFrames++;world.render(previous,state,1,cameraMode,tilt,fov,race.nextGate,0);if(readyFrames===3)el('status').textContent='3D 장면 준비 완료';updateUI();return;}
  if(!paused&&elapsed>0.25)pause('긴 프레임 지연으로 중단 · tick을 건너뛰지 않았습니다. 재개하거나 R을 누르세요.');
  if(!paused&&source==='gamepad'&&!padInput)pause('조종기 연결 또는 매핑 오류 · 비행을 멈췄습니다.');
  if(!paused){
    clock.advance(elapsed,()=>{
      if(resetPending)return;
      previous=cloneState(state);
      if(source==='keyboard'){
        if(keys.has('KeyW')||keys.has('KeyS'))hoverReference=false;
        if(hoverReference)throttle=hoverThrottle(profile,state.charge);
        else throttle=clamp(throttle+axis('KeyW','KeyS')*DT*.3,0,1);
      ...
      input=flightMode==='assisted'?assistedInput(state,pilotInput,profile):{...pilotInput};
      recorder.record(state.tick,pilotInput,input,cameraSnapshot(fov,Math.max(1,canvas.clientWidth),Math.max(1,canvas.clientHeight),devicePixelRatio));
      step(state,input,profile);
      if(collision(previous.position,state.position,track)){resetPending='충돌 또는 트랙 이탈 · 시작 위치로 초기화했습니다.';return;}
      const active=flightMode==='assisted'
        ?Math.abs(pilotInput.throttle-.5)>.02||Math.abs(pilotInput.roll)+Math.abs(pilotInput.pitch)+Math.abs(pilotInput.yaw)>.01
        :Math.abs(pilotInput.throttle-hoverThrottle(profile,state.charge))>.02||Math.abs(pilotInput.roll)+Math.abs(pilotInput.pitch)+Math.abs(pilotInput.yaw)>.01;
      if(race.update(previous.position,state.position,state.tick,active)){
        const valid=race.valid, record=recorder.finalize(race.lastSeconds!,valid);archive.add(record);
        message(valid?'랩 완료! '+time(race.lastSeconds!)+' · 같은 트랙/조종 모드 순위에 기록':'연습 랩 완료 · 설정 변경/중단으로 기록 무효');
      }
      if(state.charge<=0){resetPending='배터리 소진 · 새 배터리로 다시 시작합니다.';return;}
      if(state.tick%8===0&&state.tick!==lastGraphTick){diagnostics.sample(state,input,4*profile.maxMotorThrustN);lastGraphTick=state.tick;}
    });
    if(resetPending){const reason=resetPending;resetPending='';reset(reason);}
  }
  world.render(previous,state,paused?1:clock.alpha,cameraMode,tilt,fov,race.nextGate,Math.min(elapsed,.05));
  if(now-lastUI>70){updateUI();lastUI=now;}
}
try {
  world=createWorld(canvas,track);syncTrackUI();syncProfileUI();syncCameraTilt();el('version').textContent='PHYSICS '+PHYSICS_VERSION+' / PROFILE '+profile.version;
  world.render(previous,state,1,cameraMode,tilt,fov,0,0);updateUI();
  frameId=requestAnimationFrame(frame);
} catch(error){el('status').textContent='3D 장면 초기화 실패 · 그래픽 가속 설정을 확인해 주세요.';console.error(error);}
import.meta.hot?.dispose(()=>{
  cancelAnimationFrame(frameId);world?.dispose();
