...
export const TRAINING_TRACK: Track = {
  ...
  version: 1,
  label: '훈련장',
  description: '기존 7 m × 5 m 게이트 5개로 구성된 입문용 연습 루프',
  gates: [
...
export const RACE_TRACK: Track = {
  ...
  version: 1,
  label: '대회용 트랙',
  description: '1.8 m 정사각 게이트와 상승·하강 구간이 있는 FPV 레이스 연습 루프',
  gates: [
...
/** Swept sphere vs expanded post boxes; catches crossing between physics ticks. */
...
export function gatePassed(from: Vec3, to: Vec3, gate: Gate): boolean {
  ...
  const t = a[2] / (a[2] - b[2]);
  return Math.abs(a[0] + t*(b[0]-a[0])) < gate.width/2 - RADIUS
    && Math.abs(a[1] + t*(b[1]-a[1])) < gate.height/2 - RADIUS;
}
export function collision(from: Vec3, to: Vec3, track: Track = TRAINING_TRACK): boolean {
  if (to[1] < RADIUS || to[1] > 80 || Math.abs(to[0]) > 100 || Math.abs(to[2]) > 100) return true;
  return track.gates.some(gate => {
    const a = local(from, gate), b = local(to, gate), w = gate.width/2, h = gate.height/2;
    const posts: [Vec3, Vec3][] = [
      [[-w-.15, -h-.15, -.15], [-w+.15, h+.15, .15]],
      [[w-.15, -h-.15, -.15], [w+.15, h+.15, .15]],
      [[-w-.15, h-.15, -.15], [w+.15, h+.15, .15]],
      [[-w-.15, -h-.15, -.15], [w+.15, -h+.15, .15]],
    ];
    return posts.some(([min, max]) => segmentBox(a, b, min.map(v=>v-RADIUS) as Vec3, max.map(v=>v+RADIUS) as Vec3));
  });
}
