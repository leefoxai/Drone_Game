...
export const ASSIST_SETTINGS = { horizontalSpeedMps:4, verticalSpeedMps:2, yawRateRadS:Math.PI/3, maxHorizontalAcceleration:2.5, velocityGain:2.4, levelGain:6, verticalGain:4 };
function inverseRate(target:number,p:Profile):number {
  let low=-1,high=1;
  for(let i=0;i<24;i++){const mid=(low+high)/2;if(rate(mid,p.rates)<target)low=mid;else high=mid;}
  return (low+high)/2;
}
/** Beginner velocity/level assistance. Produces the actual input for the same 6-DoF model.
...
export function assistedInput(s:State,command:Input,p:Profile):Input {
  ...
  return {
    ...
    yaw:inverseRate(command.yaw*settings.yawRateRadS,p),
  };
}
